...
Here we will keep track of two timelines, one with no stop loss 'total' and 'dollar'
    ...
    for date, row in dataset.iterrows():
        if Higher:
            ...
            if row[sewma] / row[bewma] < 0.9965:
                ...
                stoploss.append([date, row['close'],'Sell', stop_dollar, stop_total])
                # switch over to buy condition
                Higher = not Higher

            # Look to buy in again after a stoploss. If we're still higher with +ve gradient
            #  we should buy again
            if STOP and Higher:
                if row['returns'] > 0.02:
                    # now we can buy
                    stop_total = stoploss[-1][-1] / row['close']
                    stop_dollar = stoploss[-1][-2]
                    stoploss.append([date, row['close'],'Stop_Buy', stop_dollar, stop_total])
                    STOP = False
                    high = row['close']
            ...
            if not STOP and Higher and (high / row['close'] >= stopIt):
                ...
                stop_total = stoploss[-1][-1] * row['close']
                stop_dollar = stop_total
                stoploss.append([date, row['close'],'Stop_Sell', stop_dollar, stop_total])
                STOP = True
            ...
            if row[sewma] / row[bewma] > 1.0035:
                ...
                stoploss.append([date, row['close'],'Buy', stop_dollar, stop_total])
                STOP = False
                Higher = not Higher
                high = row['close']

    price = pd.DataFrame(record, columns=('Date Close Transaction Dollar Total').split())
    stop_price = pd.DataFrame(stoploss, columns=('Date Close Transaction Stop_Dollar Stop_Total').split())
...
def plotDataset(dataset, price, stop_price, text):
    ...
    axes.plot(dataset.index, dataset['sewma'], label='ewma={}'.format(sma), color='blue')
    axes.plot(dataset.index, dataset['bewma'], label='ewma={}'.format(bma), color='red')
    axes.plot(dataset.index, dataset['close'], label='close', color='green', alpha=.5)
    axes.plot(dataset.index, dataset['longewma'], label='longma', color='orange', alpha=.5)
    axes.plot(dataset.index, dataset['high'], label='high', color='pink', alpha=.5)
    ...
    axes.scatter(stop_sold.index, stop_sold['Close'], color='r', label='Stop_Loss - Sell', lw=7, alpha=.4)
    stop_bought = pd.DataFrame(stop_price[stop_price['Transaction'] == 'Stop_Buy']['Close'])
    axes.scatter(stop_bought.index, stop_bought['Close'], color='g', label='Stop_Buy', lw=7, alpha=.4)
...
def showDollar():
    ...
    for coin in coins:
        ...
        data = pd.read_csv('Data\\MasterData\\{}\\{}_Candles_{}.csv'
                        .format(step, sym, step), index_col='MTS', parse_dates=True)
